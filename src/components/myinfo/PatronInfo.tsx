...
const MyInfo = () => {
  ...
  async function fetchData() {
    ...
    const patronId = urlParams.get("id");
    if (patronId !== null) {
      localStorage.setItem("patronId", patronId);
    } else {
      console.log("No patron exists");
    }

    try {
      if (role === "patron") {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/patron/${userId}`,
          {
            headers: {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/patron/${patronId}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
    ...
    try {
      ...
      info &&
        info.banks?.forEach(async (bank) => {
          ...
          userId && formdata.append("patronId", userId);

          const resp = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/patronBanks`,
            {
              method: "POST",
              headers: {
                Authorization: `Bearer ${token}`,
